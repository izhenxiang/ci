...
var ie = (t) => {
	...
	return (
		e = t(), (r) => {
			var a, n, o, f, x, p, d, l, j, v, u;
			return (
				p = new Set(), v = !1, l = x = o = a = d = u = void 0, n =
					new Promise(
						(i) => {
							o = i;
						},
					...
					(async () => {
						var i, s, c;
						({ id: u, host: x } = l = await te("upload/start.do?mode=oss")), i =
							"chunk_", l =
							{
					...
					async () => {
						for (var i, s, c, h, w, T, k; p.size; ) {
							T = p.values();
							for (w of T) h = b.join(d, w), Y.existsSync(h) && (
								c = await m.readFile(h), s =
									M(F({}, l), { file: new File([c], "blob") }), i =
									parseInt(ae(w)), k = (i + "").padStart(4, 0), s.name += k, s.key +=
									k, await _(() => fetch(x, R(s))), await g(h), await _(
									() =>
